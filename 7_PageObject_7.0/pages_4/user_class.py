...
class FormPage:
    ...
    def submit_form(self):
        self.submit_button = self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '.btn-outline-primary')))
        self.driver.execute_script('arguments[0].scrollIntoView({block: "center"});', self.submit_button)
        actions = ActionChains(self.driver)
        actions.move_to_element(self.submit_button).click().perform()
